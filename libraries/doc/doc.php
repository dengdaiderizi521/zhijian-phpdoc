...
/**
 * Copyright (c) 2016 ~ 2019, www.zhijian.cc Inc. All Rights Reserved
 * 至简PHP项目文档生成器
...
class Doc
{
     ...
     * @param $doc_path string 生成文档的路径
     * @param $code_path string 项目路径
     * @return bool
     */
    ...
    {
        file_put_contents($this->doc_path . '_zhijianlog/task_log.txt', '');
    }
    /**
     * 写日志
     * @param $log string 写日志
     */
    public function writeTaskLog($log)
    {
        file_put_contents($this->doc_path . '_zhijianlog/task_log.txt', $log . '<br>', 8);
    }
     ...
     * @return string
     */
    public function getTaskLog($doc_path)
    {
        $doc_path .= '_zhijianlog/';
        if (!is_dir($doc_path)) {
            if (!is_writable(dirname($doc_path))) {
                return 'DOC目录不可写';
            }
        ...
        if (!file_exists($doc_path . 'task_log.txt')) {
            return '生成失败';
        }
        return file_get_contents($doc_path . 'task_log.txt');
    }
    ...
    private function create_doc_index()
    {
        ...
        if (!empty($this->debug)) {
            ...
            copy(__DIR__ . '/temp/log.php', $this->doc_path . '/log.php');
            file_put_contents($this->doc_path . '/log.php', '<?php $logPath = "' . $this->code_path . '../logs/log-" . date("Y-m-d") . ".php"; ?>' . "\n" . file_get_contents(__DIR__ . '/temp/log.php'));
    ...
    private function get_doc_nav($doc = null, $url)
    {
        ...
        foreach ($doc as $k => $v) {
            if (is_int($k)) {
                $url = 'https://' . str_replace(array('https://', '//'), array('', '/'), $url);
                $v['path'] = 'https://' . str_replace(array('https://', '//'), array('', '/'), $v['path']);
                $nav_html .= '<li ' . ($url == $v['path'] ? ' class="Selected"' : '') . '><span><a href="' . $v['path'] . ($url == $v['path'] ? '#doc_home' : '') . '">' . $v['className'] . "</a></span>\n";
                $nav_html .= '<ul>';
    ...
    private function create_doc_file($file_path, $relative_path)
    {
        ...
        $this->writeTaskLog("正在生成：" . $doc_file_log);
        ...
        foreach ($php_code_arr as $k => $v) {
            ...
            if (preg_match('/\s?class\s/', $v) > 0 || preg_match('/\s?function\s/', $v) > 0) {
                if (preg_match('/\s?class\s/', $v) > 0) {
                    if (stripos($v, 'class') !== 0) {
                        if (preg_match('/\sclass\s/', $v) < 1) {
                            continue;
                        }
                    }
                } elseif (preg_match('/\s?function\s/', $v) > 0) {
                    if (stripos($v, 'function') !== 0) {
                        if (preg_match('/\sfunction\s/', $v) < 1) {
                            continue;
                        }
                    }
                }
                ...
                if (preg_match('/\s?function\s/', $v) > 0) {
                    ...
                    if (
                        (!$this->stat_private && preg_match('/private/', $v) > 0)
                        || (!$this->stat_protected && preg_match('/protected/', $v) > 0)
                    ) {
                        $function_start = 0;
                        $f_left_brace_count = $left;
                        $f_right_brace_count = $right;
                    ...
                    $file_html .= '<div class="panel panel-default" id="' . $fun_arr['ename'] . '_function"><div class="panel-heading"><h4 class="panel-title"><strong>' . $function_count . '. ' . $function_name . '</strong>' . (!empty($this->debug) && preg_match('/(private)|(protected)/',
                            $v) === 0 ? '<a class="pull-right" target="_debug" href="' . $this->doc_url . '/form.php?class=' . $class_name . '&method=' . $name . '&relative_path=' . $relative_path . '&file=' . $relative_path . $file_path . '&name=' . $function_name . '"><i class="fa fa-wrench"></i>测试</a>' : '') . '</h4></div><div class="panel-body">';
                    ...
                    $function_count++;
                    if (preg_match('/\s?function\s/', $v) > 0 && preg_match('/(private)|(protected)/',
                            $v) === 0 && !empty($class_name) && $class_start == 1 && empty($code_arr['url'])
                    ) {
                        $file_html .= '<div class="doc_block"><label class="label label-info">链接:</label> <span style="color: #0066cc;">' . $this->base_url . $relative_path . strtolower($class_name) . '/' . strtolower($name) . '</span></div>';
                    ...
                    if (!empty($param_file_contents['url']) && !empty($this->debug)) {
                        ...
                        $param_file_contents['method'] = empty($code_arr['method_str']) ? 'POST' : $code_arr['method_str'];
                        $param_file_contents['params'] = $params;
                        $param_file_contents['brief'] = !empty($code_arr['brief_text']) ? $code_arr['brief_text'] : '';
                        $this->write_file($this->doc_path . '/params/' . $relative_path . $file_path,
                            '$params[\'' . $class_name . '/' . $name . '\'] = ' . var_export($param_file_contents,
                                true) . ";\n");
                    }
                } else {
                    $class_name = trim(preg_replace('/^.*class\s+([A-Za-z0-9_]*)(([\s]+)|\{).*/', '$1', $v));
                    $file_html .= '<div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title"><strong>模块：' . (! empty($code_arr['name']) ? $code_arr['name'] : $class_name) . '</strong></h4></div><div class="panel-body">' . "\n";
                    if (!empty($code_arr['brief'])) {
                        $file_html .= $code_arr['brief'] . "\r\n";
                        unset($code_arr['brief']);
                    }
                    ...
                    if (empty($showClassName)) {
                        $showClassName = (! empty($code_arr['name']) ? $code_arr['name'] : $class_name);
                    }
        ...
        $doc['className'] = $showClassName;
        $doc['path'] = $this->doc_docs_url . $relative_path . strtolower(preg_replace('/^(?:.*\/)?([^.]+).*$/',
                '$1' . $ext, basename($file_path)));
        ...
        file_put_contents($this->doc_docs_path . '/' . $relative_path . strtolower(preg_replace('/^(?:.*\/)?([^.]+).*$/',
                '$1' . $ext, basename($file_path))), '<div id="doc_home" class="col-md-12">' . $file_html . '</div>');
        return $doc;
    }
    ...
    private function filter_doc(&$code_arr)
    {
        ...
        if (!empty($code_arr)) {
            ...
            if (isset($code_arr['scene'])) {
                $arr['scene'] = '<div class="doc_block"><label class="label label-info">场景:</label><br>' . implode("<br>",
                        $code_arr['scene']) . '</div>';
            }
            if (isset($code_arr['method'])) {
                $arr['method_str'] = $code_arr['method'][0];
                $arr['method'] = '<div class="doc_block"><label class="label label-info">请求:</label> ' . implode(" ",
                        $code_arr['method']) . '</div>';
            }
            if (isset($code_arr['date'])) {
                $arr['date'] = '<div class="doc_block"><label class="label label-info">时间:</label> ' . implode(" ",
                        $code_arr['date']) . '</div>';
            }
            if (isset($code_arr['author'])) {
                $arr['author'] = '<div class="doc_block"><label class="label label-info">作者:</label> ' . implode(" ",
                        $code_arr['author']) . '</div>';
            }
            if (isset($code_arr['version'])) {
                $arr['version'] = '<div class="doc_block"><label class="label label-info">版本:</label> ' . implode(" ",
                        $code_arr['version']) . '</div>';
            }
            if (isset($code_arr['copyright'])) {
                $arr['copyright'] = '<div class="doc_block"><label class="label label-info">版权:</label> ' . implode("<br>",
                        $code_arr['copyright']) . '</div>';
            }
            if (isset($code_arr['url'])) {
                ...
                foreach ($code_arr['url'] as $k => $v) {
                    $arr['url'] .= ' <span style="color: #0066cc;">' . $v . '</span>';
                }
                $arr['url'] .= '</div>';
            }
            ...
            if (isset($code_arr['brief'])) {
                $arr['name'] = implode("<br>", $code_arr['brief']);
//                $arr['brief_text'] = implode("<br>", $code_arr['brief']);
            }
            if (isset($code_arr['desc'])) {
                $arr['brief'] = implode("<br>", $code_arr['desc']);
                $arr['brief_text'] = implode("<br>", $code_arr['desc']);
            }
    ...
    private function get_doc_params($arr)
    {
        ...
        foreach ($arr as $k => $v) {
            if (preg_match('/^@param\s+([^\s]*)\s+([^\s]*)\s+([^\s]*)\s*(.*)$/', strtolower($v), $t) > 0) {
                if (stristr($t[1], '$') !== false) {
                    $param_name = ltrim($t[1], '$');
                }
                if (empty($param_name)) {
                    continue;
                }
                $params[$param_name]['type'] = $t[2];
                switch ($t[2]) {
                    case 'integer':
                    case 'int':
                ...
                $params[$param_name]['mandatory'] = in_array($t[3], ['null', 'choice']) ? 0 : 1;
                $params[$param_name]['text'] = (in_array($t[3], ['null', 'choice']) ? $t[4] : $t[3]);
